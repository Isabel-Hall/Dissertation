...
try:
    experiment_id = mlflow.create_experiment("testing_baseline_perceptual")
    experiment = mlflow.get_experiment(experiment_id)
except:
    experiment = mlflow.get_experiment_by_name("testing_baseline_perceptual")
...
model.load_state_dict(
    torch.load("/home/issie/code/py/MRI/src/artifacts/15/ee182b6dba1d4f7090b7358c0fbb7f61/States/model-99.state")
)
coarse_model = Coarse_net(input_channels=SEQ_LENGTH).float()
coarse_model.load_state_dict(
    torch.load("/home/issie/code/py/MRI/src/artifacts/15/ee182b6dba1d4f7090b7358c0fbb7f61/States/coarse_model-99.state")
)
...
with mlflow.start_run(experiment_id=experiment_id) as run:
    ...
    for i, (batch_of_data, ground_truth) in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        torch.save(t1_difference, f"./results_torch/only_percep/{i}_t1.pt")
        torch.save(t2_difference, f"./results_torch/only_percep/{i}_t2.pt")
        ...
        ax[0].set_title("Perceptual loss \nT1 error")
        im = ax[1].matshow(t2_rel_diff, vmin=0, vmax=100)
        ax[1].set_title("Perceptual loss \nT2 error")
        fig.colorbar(im, ax=ax.ravel().tolist(), shrink=0.5)
        ax[0].axis("off")
        ax[1].axis("off")
        plt.savefig(f"./results_images/baseline/{i}only_percep.png", bbox_inches="tight")
        plt.close(fig)
        ...
        ax2.set_title("Perceptual loss \nT1 error")
        fig.colorbar(im1, ax=ax1, shrink=0.5)
        fig.colorbar(im2, ax=ax2, shrink=0.5)
        ax1.axis("off")
        ax2.axis("off")
        plt.savefig(f"./results_images/abs{i}only_percep.png", bbox_inches="tight")
        plt.close(fig)
    ...
    print("times total", times_np.sum(), "mean=", times_np.mean()) #0.5797171592712402 mean= 0.03864781061808268
    print("dropping first element")
    times_short_np = times_np[1:]
    print("times total", times_short_np.sum(), "mean=", times_short_np.mean())
    # total 0.35065317153930664 mean= 0.025046655109950473