...
try:
    experiment_id = mlflow.create_experiment("testing_updated_baseline_unet")
    experiment = mlflow.get_experiment(experiment_id)
except:
    experiment = mlflow.get_experiment_by_name("testing_updated_baseline_unet")
...
model.load_state_dict(
    torch.load("/home/issie/code/py/MRI/src/artifacts/6/67f77b58846546cf80a9dfbdf6d66cba/States/model-99.state")
)
model.eval()
coarse_model = Coarse_net().float()
coarse_model.load_state_dict(
    torch.load("/home/issie/code/py/MRI/src/artifacts/6/67f77b58846546cf80a9dfbdf6d66cba/States/coarse_model-99.state")
)
coarse_model.eval()
...
with mlflow.start_run(experiment_id=experiment_id) as run:
    ...
    for i, (batch_of_data, ground_truth) in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        torch.save(t1_difference, f"./results_torch/rnn_updated/{i}_t1.pt")
        torch.save(t2_difference, f"./results_torch/rnn_updated/{i}_t2.pt")
        ...
        ax[0].set_title("Compression RNN \nT1 error")
        im = ax[1].matshow(t2_rel_diff, vmin=0, vmax=100)
        ax[1].set_title("Compression RNN \nT2 error")
        fig.colorbar(im, ax=ax.ravel().tolist(), shrink=0.5)
        ax[0].axis("off")
        ax[1].axis("off")
        plt.savefig(f"./results_images/baseline/{i}rnn.png", bbox_inches="tight")
        plt.close(fig)

        fig, (ax1, ax2) = plt.subplots(1, 2)
        plt.tight_layout()
        im1 = ax1.matshow(t1_difference)
        ax1.set_title("Compression RNN \nT1 error")
        im2 = ax2.matshow(t2_difference)
        ax2.set_title("Compression RNN \nT1 error")
        fig.colorbar(im1, ax=ax1, shrink=0.5)
        fig.colorbar(im2, ax=ax2, shrink=0.5)
        ax1.axis("off")
        ax2.axis("off")
        plt.savefig(f"./results_images/abs{i}rnn.png", bbox_inches="tight")
        plt.close(fig)
    ...
    print("times total", times_np.sum(), "mean=", times_np.mean()) # 0.40294337272644043 mean= 0.026862891515096028
    print("dropping first element")
    times_short_np = times_np[1:]
    print("times total", times_short_np.sum(), "mean=", times_short_np.mean())
    # total 0.34758853912353516 mean= 0.024827752794538225
