...
try:
    experiment_id = mlflow.create_experiment("testing_perceptual_conv1d_unet")
    experiment = mlflow.get_experiment(experiment_id)
except:
    experiment = mlflow.get_experiment_by_name("testing_perceptual_conv1d_unet")
...
model.load_state_dict(
    torch.load("/home/issie/code/py/MRI/src/artifacts/17/76f341356c0a457e86c2d2afd0c9f060/States/model-99.state")
)
model.eval()
coarse_model = CoarseModel(input_channels=SEQ_LENGTH).float()
coarse_model.load_state_dict(
    torch.load("/home/issie/code/py/MRI/src/artifacts/17/76f341356c0a457e86c2d2afd0c9f060/States/coarse_model-99.state")
)
coarse_model.eval()
...
with mlflow.start_run(experiment_id=experiment_id) as run:
    ...
    for i, (batch_of_data, ground_truth) in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        torch.save(t1_difference, f"./results_torch/noisy_percep/{i}_t1.pt")
        torch.save(t2_difference, f"./results_torch/noisy_percep/{i}_t2.pt")
        ...
        ax[0].axis("off")
        ax[1].axis("off")
        plt.savefig(f"./results_images/baseline/{i}noisy_percep.png", bbox_inches="tight")
        plt.close(fig)
        ...
        ax1.axis("off")
        ax2.axis("off")
        plt.savefig(f"./results_images/abs{i}noisy_percep.png", bbox_inches="tight")
        plt.close(fig)
    ...
    print("times total", times_np.sum(), "mean=", times_np.mean()) # 1.2692275047302246 mean= 0.08461516698201497
    print("dropping first element")
    times_short_np = times_np[1:]
    print("times total", times_short_np.sum(), "mean=", times_short_np.mean())
    # total 0.3347904682159424 mean= 0.02391360487256731
