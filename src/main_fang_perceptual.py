...
try:
    experiment_id = mlflow.create_experiment("baseline_unet_w_only_perceptual")
    experiment = mlflow.get_experiment(experiment_id)
except:
    experiment = mlflow.get_experiment_by_name("baseline_unet_w_only_perceptual")
...
coarse_model = Coarse_net(input_channels=SEQ_LENGTH).float() # Compression model
autoencoder = AutoEncoder(2, 2).float() # Pre-trained autoencoder
autoencoder.load_state_dict(
    torch.load("/home/issie/code/py/MRI/src/artifacts/autoencoder_stuff/autoencoder_3.state")
)
...
with mlflow.start_run(experiment_id=experiment_id) as run:
    ...
    for epoch in range(N_EPOCHS):
        for i, (batch_of_data, ground_truth) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            total_loss.backward() # backwards pass
