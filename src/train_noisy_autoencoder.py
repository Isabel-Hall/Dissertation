...
from data.noisy_groundtruth_dataset import GroundTruthDataset
...
for epoch in tqdm(range(1000)):
    ...
    for i, (label, noisy_map) in enumerate(dataloader):

        label = label.cuda().float()
        noisy_map = noisy_map.cuda().float()
        ...
        optimiser.zero_grad()
        reconstructed_label, z = autoencoder(noisy_map)
    ...
    plt.savefig("./artifacts/autoencoder_stuff/noisy_reconstruction_loss_4.png")
    plt.close(fig)
    ...
    plt.savefig("./artifacts/autoencoder_stuff/noisy_image_4.png")
    plt.close(fig)

    if epoch % 111 == 0:
        torch.save(autoencoder.state_dict(), f"./artifacts/autoencoder_stuff/noisy_autoencoder_4.state")
