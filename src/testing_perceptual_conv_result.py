...
model.load_state_dict(
    torch.load("/home/issie/code/py/MRI/src/artifacts/13/d9d3c2eaedf344b49d1b22326490af79/States/model-99.state")
)
model.eval()
coarse_model = CoarseModel(input_channels=SEQ_LENGTH).float()
coarse_model.load_state_dict(
    torch.load("/home/issie/code/py/MRI/src/artifacts/13/d9d3c2eaedf344b49d1b22326490af79/States/coarse_model-99.state")
)
coarse_model.eval()
...
with mlflow.start_run(experiment_id=experiment_id) as run:
    ...
    for i, (batch_of_data, ground_truth) in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        torch.save(t1_difference, f"./results_torch/percep_conv/{i}_t1.pt")
        torch.save(t2_difference, f"./results_torch/percep_conv/{i}_t2.pt")
        ...
        ax[0].axis("off")
        ax[1].axis("off")
        plt.savefig(f"./results_images/baseline/{i}percep_conv.png", bbox_inches="tight")
        plt.close(fig)
        ...
        ax1.axis("off")
        ax2.axis("off")
        plt.savefig(f"./results_images/abs{i}percep_conv.png", bbox_inches="tight")
        plt.close(fig)
    ...
    print("times total", times_np.sum(), "mean=", times_np.mean()) # 0.5750148296356201 mean= 0.038334321975708005
    print("dropping first element")
    times_short_np = times_np[1:]
    print("times total", times_short_np.sum(), "mean=", times_short_np.mean())
    # total 0.3423616886138916 mean= 0.024454406329563687
